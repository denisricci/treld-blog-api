...
public class PostController {
    ...
    @ApiOperation(value="findPostByTags", nickname="findPostByTags", notes = "send tags separated by \',\'")
	@RequestMapping(value = PATH_TAGS, method = RequestMethod.GET)
	public ResponseEntity<List<Post>> findByTags(@RequestParam("tags") String tagsParam, @RequestParam("page") int page) {

        List<String> tags = Arrays.asList(tagsParam.split(","));

        return ResponseEntity.ok(postService.findByTags(tags, page));
	}
